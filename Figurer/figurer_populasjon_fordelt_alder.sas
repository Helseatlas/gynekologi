...
proc sgplot data=SSB_i_alder noborder noautolegend sganno=annossb pad=(Bottom=5%);
  ...
  yaxis &yvalues label="Number" 
	...
	xaxis values=(16 to 105 by 1) fitpolicy=thin offsetmin=0.035 
          label="Age" labelattrs=(size=&fontst) valuesformat=&yformat. valueattrs=(size=&fontst);
...
proc sgplot data=NPR_utv_SSB_alder noborder noautolegend sganno=anno pad=(Bottom=5%);
  vbarparm category=alder response=innbyggere / fillattrs=(color=CX95BDE6) outlineattrs=(color=grey thickness=0) missing name="hp1" legendlabel="Number of women in Norway";
  vbarparm category=alder response=personer / fillattrs=(color=CX568BBF) outlineattrs=(color=grey thickness=0) missing name="hp2" legendlabel="Number in specialist health service";
  vbarparm category=alder response=tot_unik / fillattrs=(color=CX00509E) outlineattrs=(color=grey thickness=0) missing name="hp3" legendlabel="Number in the Gynaecology Healthcare Atlas";
